...
1. Подключитесь к СУБД через psql и создайте базу данных skillbox_db, которая должна быть доступна юзеру, 
2. указанному в env-переменных.
...
4. С помощью функции `before_first_request`
 создайте десять пользователей и десять сортов кофе. 
    - Для этого воспользуйтесь сервисом генерации тестового api-ответа и заполнения атрибута address у 
    - пользователей — [DummyJSON](https://dummyjson.com/users).
    - Для кофе — https://dummyjson.com/products/search?q=coffee.
    - Обработайте ответ получения кофе, где `title=title`, `category=category`, `description=description`, 
...
5. Создайте роуты для выполнения запросов (название не играет роли).
    ...
    - [полнотекстовый поиск](https://docs.sqlalchemy.org/en/14/dialects/postgresql.html#full-text-search), 
    - название — входной параметр). 
...
.address — JSON. Ознакомьтесь со 
[статьёй](https://postgrespro.ru/docs/postgrespro/9.5/datatype-textsearch) по поводу текстового поиска. 
...
6. Сделайте имитацию конфликтной ситуации: создайте миграцию по добавлению user.patronomic, в 
7. атрибуте down_revision укажите миграцию из п. 2.
